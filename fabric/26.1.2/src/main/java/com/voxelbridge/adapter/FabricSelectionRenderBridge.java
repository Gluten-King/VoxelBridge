...
/**
 * Fabric selection renderer using the MC 26.1 Gizmo line primitives.
 * <p>
 * The 4th argument to {@link DrawableGizmoPrimitives#render} is the camera
 * view-rotation matrix (see LevelRenderer line ~565-566:
 * {@code camera.getViewRotationMatrix(modelViewMatrix)}), used internally for
 * near-plane line clipping. We obtain it from
 * {@code CameraRenderState.viewRotationMatrix}.
 */
    ...
    private static void emitChunkStatus(GizmoPrimitives p, BlockPos pos1, BlockPos pos2) {
        ...
        for (Map.Entry<Long, ChunkState> entry : states.entrySet()) {
            ...
            if (state == ChunkState.DONE) {
                color = 0xFF19E619; // green
            } else if (state == ChunkState.RUNNING) {
                color = 0xFFFFCC19; // yellow
            } else {
                color = 0xFFFF3333; // red
            }
            // Slightly transparent so overlapping chunk boxes don't fully obscure each other
