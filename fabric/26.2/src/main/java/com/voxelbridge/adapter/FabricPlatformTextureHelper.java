    ...
    private ResolvedTexture resolvePainting(Painting painting) {
        try {
            ...
            TextureAtlasSprite sprite = Minecraft.getInstance()
                .getAtlasManager()
                .getAtlasOrThrow(net.minecraft.data.AtlasIds.PAINTINGS)
                .getSprite(assetId);
            if (sprite != null && sprite.contents() != null
                    && !sprite.contents().name().toString().contains("missingno")) {
                Identifier spriteName = normalizePaintingSpriteName(sprite.contents().name());
            ...
            String path = assetId.getPath().startsWith("painting/")
                ? assetId.getPath()
                : "painting/" + assetId.getPath();
