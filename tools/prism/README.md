...
The launcher root, Modrinth profile root, and Java 17/21/25 executables can all be
...
```powershell
& 'D:\PrismLauncher-Windows-MinGW-w64-Portable-11.0.3\prismlauncher.exe' --launch vb-fabric-1.21.11-continuity
```

Open an instance's Prism page without launching Minecraft:

```powershell
& 'D:\PrismLauncher-Windows-MinGW-w64-Portable-11.0.3\prismlauncher.exe' --show vb-fabric-1.21.11-continuity
```

Synchronize the manually maintained RestWorld test scene and its selected
resource packs into every matrix instance. All Fabric instances use Continuity
and its bundled CTM packs:
...
hand-maintained source is the Fabric 1.20.1 Continuity instance and remains at
...
unless `-Refresh` is supplied; a refresh verifies a temporary copy before
deleting and replacing the previous target world.
...
Force a multi-page atlas compatibility run with the smallest page that fits
the current RestWorld textures (the ender chest texture is 512x512):

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\prism\Run-PrismRestWorldMatrix.ps1 -AtlasSize 512 -SkipBlender
```

Check collected glTF files for primitives whose bound atlas image disagrees
with the page encoded in their UV coordinates:

```powershell
.\tools\prism\Measure-GltfAtlasPageBindings.ps1 -Path .\build\prism-restworld-runs\<run-id>
```
