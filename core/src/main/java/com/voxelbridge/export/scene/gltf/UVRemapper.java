        ...
        try (FileChannel geometryChannel = FileChannel.open(geometryBin, StandardOpenOption.READ);
            ...
            while (processedQuads < totalQuads) {
                ...
                for (int i = 0; i < quadsThisChunk; i++) {
                    ...
                    // Remap uv0
                    if (UvRemapUtil.shouldRemap(state, spriteKey, options)) {
                        for (int v = 0; v < 4; v++) {
                            float[] remapped = UvMapper.remapUv(state, spriteKey, uv0[v * 2], uv0[v * 2 + 1], options);
                            uv0[v * 2] = remapped[0];
                    ...
                    boolean isColormapMode = UvRemapUtil.isColormapMode(options);
                    if (!isColormapMode && UvRemapUtil.shouldRemap(state, overlayKey, options)) {
                        boolean hasUV1 = false;
                        for (float f : uv1) if (f != 0) { hasUV1 = true; break; }
                        if (hasUV1) {
                            for (int v = 0; v < 4; v++) {
                                float[] remapped = UvMapper.remapUv(state, overlayKey, uv1[v * 2], uv1[v * 2 + 1], options);
                                uv1[v * 2] = remapped[0];
