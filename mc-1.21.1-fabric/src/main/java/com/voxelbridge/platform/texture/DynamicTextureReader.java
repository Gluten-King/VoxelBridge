...
final class DynamicTextureReader {
    ...
    static BufferedImage tryRead(Identifier location) {
        try {
            if (location == null) {
                return null;
            }
            var manager = ClientAccessHolder.get().getTextureManager();
            if (manager == null) {
                return null;
            }
            ...
            BufferedImage fromFile = readFileTexture(texture);
            if (fromFile != null) {
                return fromFile;
            }
    ...
    private static BufferedImage readFileTexture(AbstractTexture texture) {
        File file = findFileField(texture);
        if (file == null || !file.isFile()) {
            return null;
        }
        try {
            ...
            VoxelBridgeLogger.warn(LogModule.TEXTURE_RESOLVE,
                String.format("[DynamicTextureReader][WARN] Failed to read texture file %s: %s", file, e.getMessage()));
            return null;
        }
    }
