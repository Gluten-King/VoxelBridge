    ...
    public BlockEntityExportResult export(
        ...
        if (baked != null) {
            ...
            VoxelBridgeLogger.warn(LogModule.BLOCKENTITY,
                "[BannerBlockEntityHandler][WARN] Missing baked image for " + textures.bakedHandle().spriteKey());
        }
        ...
        if (blockEntityHandle != null) {
            ...
            overrideWrapper = new TextureOverrideMap() {
                @Override
                public com.voxelbridge.export.texture.EntityTextureManager.TextureHandle resolve(net.minecraft.util.Identifier spriteName) {
                    return delegate.resolve(spriteName) != null ? finalHandle : null;
                }

                @Override
                public boolean skipQuad(net.minecraft.util.Identifier spriteName, float[] localU, float[] localV) {
