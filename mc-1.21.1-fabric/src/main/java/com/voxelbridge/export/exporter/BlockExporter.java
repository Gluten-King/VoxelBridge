...
public final class BlockExporter {
    ...
    public void sampleBlock(BlockState state, BlockPos pos) {
        ...
        Vec3d randomOffset = vanillaRandomTransformEnabled ? state.getModelOffset(level, pos) : Vec3d.ZERO;
    ...
    private void detectCtmOverlaysByPosition(List<BakedQuad> quads, BlockState state, BlockPos pos, String blockKey, Vec3d randomOffset) {
    ...
    private boolean isNeighborSolid(BlockPos neighbor) {
        ...
        if (ExportRuntimeConfig.isFillCaveEnabled()) {
            if (state.isAir() && level.getLightLevel(LightType.SKY, neighbor) == 0) {
