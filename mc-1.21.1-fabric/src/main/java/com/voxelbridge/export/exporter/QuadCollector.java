...
final class QuadCollector implements VertexConsumer {
    ...
    QuadCollector(IrSink sink, ExportContext ctx, BlockPos pos, Sprite[] sprites,
    ...
    public void vertex(float x, float y, float z, int color, float u, float v, int overlay, int light, float normalX, float normalY, float normalZ) {
        ...
        color(r, g, b, a);
        texture(u, v);
        overlay(overlay & 0xFFFF, (overlay >>> 16) & 0xFFFF);
        light(light & 0xFFFF, (light >>> 16) & 0xFFFF);
    ...
    @Override
    public VertexConsumer overlay(int uv) { return this; }

    @Override
    public VertexConsumer light(int uv) { return this; }
    ...
    private void emitQuad() {
        Sprite sprite = chooseSpriteForQuad();
        ...
        float[] normalizedUVs = GeometryUtil.normalizeUVs(
            uvs, sprite.getMinU(), sprite.getMaxU(), sprite.getMinV(), sprite.getMaxV());
    ...
    private Sprite chooseSpriteForQuad() {
    ...
    private boolean isPointInSprite(float u, float v, Sprite s) {
        return u >= s.getMinU() && u <= s.getMaxU() && v >= s.getMinV() && v <= s.getMaxV();
    }
    
    private float distToCenter(Sprite s, float u, float v) {
        float cu = (s.getMinU()+s.getMaxU())*0.5f;
        float cv = (s.getMinV()+s.getMaxV())*0.5f;
