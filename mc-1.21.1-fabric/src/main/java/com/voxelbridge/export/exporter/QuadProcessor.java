...
public final class QuadProcessor {
    ...
    public QuadProcessor(ExportContext ctx, World level, IrSink sceneSink,
    ...
    public void processQuad(BlockState state, BlockPos pos, BakedQuad quad,
                            String blockKey, Vec3d randomOffset) {
        ...
        boolean doubleSided = state.getBlock() instanceof PlantBlock;
    ...
    private int computeTintColor(BlockState state, BlockPos pos, BakedQuad quad) {
        if (quad.getColorIndex() < 0) return -1;
        return ClientAccessHolder.get().getMinecraft().getBlockColors().getColor(state, level, pos, quad.getColorIndex());
    }
     ...
     */
    private void ensurePbrTexturesCached(Sprite sprite, String spriteKey) {
