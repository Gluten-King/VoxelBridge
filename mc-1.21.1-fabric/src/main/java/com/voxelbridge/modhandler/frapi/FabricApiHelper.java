...
public final class FabricApiHelper {
    ...
    private static BakedQuad toBakedQuad(QuadView quad, SpriteFinder spriteFinder) {
        int vertexSize = 8; // DefaultVertexFormat.BLOCK stride
        ...
        for (int i = 0; i < 4; i++) {
            ...
            vertices[offset + 3] = argbToAbgr(quad.color(i));
            ...
            if (quad.hasNormal(i)) {
                ...
                if (dir != null) {
                    vertices[offset + 7] = packNormal(dir.getOffsetX(), dir.getOffsetY(), dir.getOffsetZ());
                } else {
