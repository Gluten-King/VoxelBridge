...
public final class ProgressNotifier {
    ...
    public static void renderOverlay(Minecraft mc, GuiGraphicsExtractor gfx) {
        ...
        } else if (current.stage() == ExportProgressTracker.Stage.IDLE) {
            // Closing an export session replaces the tracker state with IDLE. Keep a
            // completed overlay long enough to finish its hold/fade animation, but
            // discard any non-terminal overlay left behind by an error or abort.
            if (!shouldRetainCompletedOverlay(lastProgress.stage(), current.stage())) {
                reset();
                return;
            }
        } else {
    ...
    static boolean shouldRetainCompletedOverlay(ExportProgressTracker.Stage displayedStage,
                                                ExportProgressTracker.Stage trackerStage) {
        return displayedStage == ExportProgressTracker.Stage.COMPLETE
            && trackerStage == ExportProgressTracker.Stage.IDLE;
    }
