...
final class QuadCollector implements VertexConsumer {
    ...
    private void emitQuad() {
        ...
        if (!emittedPlanes.add(planeKey(positions, normal))) {
            resetQuadState();
            return;
        }

        String spriteKey = ctx.resolveSpriteKey(sprite);
    ...
    private static boolean isLavaSprite(String spriteKey, String materialKey) {
        String sprite = spriteKey != null ? spriteKey.toLowerCase(java.util.Locale.ROOT) : "";
        String material = materialKey != null ? materialKey.toLowerCase(java.util.Locale.ROOT) : "";
        if (sprite.contains("gel") || material.contains("gel")) return false;
        return sprite.contains("lava") || material.contains("lava");
    }
