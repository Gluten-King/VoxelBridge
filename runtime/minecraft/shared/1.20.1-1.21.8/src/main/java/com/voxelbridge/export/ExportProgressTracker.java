...
/**
 * UI-facing progress façade. Mutable maps and counters live in a session-owned
 * {@link ExportProgressState}; only the active-session pointer remains global.
 */
public final class ExportProgressTracker {
    ...
    private static volatile ExportProgressState active = new ExportProgressState();

    private ExportProgressTracker() {}

    public static synchronized void bind(ExportSession session) {
        if (session == null) return;
        ExportProgressState bound = session.computeAttribute(SESSION_KEY, () -> active);
        active = bound;
        session.onClose(() -> release(bound));
    }

    private static synchronized void release(ExportProgressState bound) {
        if (active == bound) active = new ExportProgressState();
    }
    ...
    public record Progress(int done,
                           int failed,
                           int total,
                           int running,
                           long startNanos,
