    ...
    private static TextureAtlasSprite[] getFluidSprites(ExportContext ctx,
        ...
        try {
            ...
            for (String[] pair : pairs) {
                ResourceLocation stillLoc = new ResourceLocation(namespace, pair[0]);
                ResourceLocation flowLoc = new ResourceLocation(namespace, pair[1]);
                TextureAtlasSprite still = atlas.getSprite(stillLoc);
                TextureAtlasSprite flow = atlas.getSprite(flowLoc);
                String stillKey = ctx.resolveSpriteKey(still);
                String flowKey = ctx.resolveSpriteKey(flow);
