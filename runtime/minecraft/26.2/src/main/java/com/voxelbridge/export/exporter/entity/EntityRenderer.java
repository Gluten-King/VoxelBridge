...
import com.voxelbridge.export.exporter.capture.CapturedTextTextureSupport12111;
...
public final class EntityRenderer {
    ...
    private static class CaptureBuffer extends CaptureBufferBase {
        ...
        @Override
        public void onQuad(RenderType renderType, RenderCapture.Submission submission,
            ...
            CapturedTextTextureSupport12111.logUvOnce(ctx, "entity", "EntityRenderer", renderType, uvStats);
            uvStats = MapDecorationUvStrategy12111.normalize(ctx, uvStats, renderType);
            String materialGroupKey = MaterialGroupKey.entity(entity);
            if (CapturedTextTextureSupport12111.isTextRenderType(renderType)) {
                materialGroupKey = MaterialSemantic.glyph(materialGroupKey);
            }
            RenderCapture.Submission effectiveSubmission = submission;
            if (entity instanceof net.minecraft.world.entity.decoration.painting.Painting
                && (effectiveSubmission == null
                    || effectiveSubmission.uvSpace() == BufferSource.UvSpace.UNKNOWN)) {
                // PaintingRenderer submits custom geometry through the generic buffer,
                // but its vertices already contain TextureAtlasSprite.getU/getV values.
                // Treating those values as sprite-local remaps the front, back and edge
                // texels a second time and produces the characteristic narrow UV island.
                TextureAtlasSprite submittedSprite = effectiveSubmission != null
                    ? effectiveSubmission.sprite() : null;
                effectiveSubmission = new RenderCapture.Submission(
                    submittedSprite, BufferSource.UvSpace.ATLAS);
            }
            updateBucketKey();
            CapturedQuadProcessor.process(
                ...
                materialGroupKey,
                this::resolveTexture,
                CapturedTextTextureSupport12111::writeEntityUvs,
        ...
        private CapturedQuadProcessor.TextureResult resolveTexture(
            ...
            RenderCapture.Submission submission,
            RenderCaptureUtil.UvStats uvStats,
            float[] positions
        ) {
            Identifier rtTexture = renderType != null ? RenderTypeTextureResolver.INSTANCE.resolve(renderType) : null;
            if (rtTexture == null) {
                CapturedTextTextureSupport12111.logMissingTextureOnce(
                    ctx, "entity", "EntityRenderer", renderType);
            }
            ...
            ResolvedTexture textureRes = ResolvedTexture.fromSprite(submission.sprite());
            if (textureRes == null) {
                textureRes = resolver != null ? resolver.resolve(source, renderType) : null;
            }
            if (textureRes == null && CapturedTextTextureSupport12111.isTextRenderType(renderType) && rtTexture != null) {
                textureRes = new ResolvedTexture(rtTexture, 0f, 1f, 0f, 1f, false, null, null);
            }
            textureRes = CapturedTextTextureSupport12111.resolveFallback(
                ctx, renderType, uvStats, textureRes, "EntityRenderer");

            RenderCaptureUtil.UvStats atlasUvStats = uvStats;
            if (textureRes != null && textureRes.isAtlasTexture() && textureRes.sprite() == null) {
                textureRes = RenderCaptureUtil.resolveAtlasSprite(textureRes, atlasLocator(ctx), atlasUvStats, textureRes.atlasLocation());
            }

            if (submission.sprite() == null
                && source instanceof net.minecraft.world.entity.decoration.painting.Painting painting
                && textureRes != null && textureRes.isAtlasTexture() && textureRes.sprite() != null) {
                textureRes = PaintingTextureStrategy12111.select(
            ...
            if (textureRes != null && textureRes.texture() != null) {
                ...
                if (source instanceof AbstractClientPlayer player) {
                    handle = PlayerAttachmentTextureStrategy12111.register(ctx, player, textureRes.texture());
                }
                if (handle == null) {
