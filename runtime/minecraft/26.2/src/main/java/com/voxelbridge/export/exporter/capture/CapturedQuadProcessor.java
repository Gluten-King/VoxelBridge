...
public final class CapturedQuadProcessor {
    ...
    public static void fillPositionsAndColors(List<RenderCapture.Vertex> verts, float[] positions, float[] colors) {
        ...
        // The shared IR stores quads. Represent triangles as a degenerate quad
        // by repeating the final vertex instead of leaving vertex 4 at origin.
        if (count == 3) {
            System.arraycopy(positions, 6, positions, 9, 3);
            System.arraycopy(colors, 8, colors, 12, 4);
        }
    }
    ...
    public static <T> void process(
        ...
        RenderCapture.Submission captureSubmission = submission != null
            ? submission : RenderCapture.Submission.UNKNOWN;
        TextureResult result = textureHandler.resolve(
            ctx, source, renderType, captureSubmission, stats, positions);
        ...
        // Some renderer paths emit sprite-local UVs in ~[0,1], while submissions
        // with an explicit sprite are atlas-expanded by SpriteCoordinateExpander.
        // ResolvedTexture still carries the MC sheet sub-rect, so distinguish the
        // two forms before un-expanding UVs (small faces and locks are local too).
        if (captureSubmission.uvSpace() == com.voxelbridge.platform.render.capture.BufferSource.UvSpace.SPRITE_LOCAL) {
            useAtlasUv = false;
            u0 = 0f; u1 = 1f;
            v0 = 0f; v1 = 1f;
        } else if (useAtlasUv && textureRes != null
                && captureSubmission.uvSpace() != com.voxelbridge.platform.render.capture.BufferSource.UvSpace.ATLAS) {
